...
void read_cl_file(char* path)
{
	...
	char *pwd = strcat(path,"/Cfiles/covariance.cl");
	fp = fopen(pwd, "r");
	if (!fp) {
...
{	
   ...
   // Connect to a compute device
   	...
   	printf(" The program covariance will execute on : %s \n", query);
...
{	
	...
	init_arrays(m, n, POLYBENCH_ARRAY(data)); // M*N
