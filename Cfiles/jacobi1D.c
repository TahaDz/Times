...
void read_cl_file(char* path)
{
	...
	char *pwd = strcat(path,"/Cfiles/jacobi1D.cl");
	fp = fopen(pwd, "r");
	if (!fp) {
...
{	
   ...
   // Connect to a compute device
   	...
   	printf(" The program jacobi1D will execute on : %s \n", query);
...
{	
	...
	init_array(n, POLYBENCH_ARRAY(a), POLYBENCH_ARRAY(b)); //N+N
	...
	printf("Execution Time in seconds: ");
 	...
 	FILE *myfile ;
   	...
   	fprintf(myfile,"%s","jacobi1D");
   	fprintf(myfile,"|");
   	fprintf(myfile,"%0.6lf",executionTime);
   	fprintf(myfile,"|");
   	fprintf(myfile,"%d",N+N);
   	fprintf(myfile,"\n");
   	fclose (myfile);
