...
void read_cl_file(char* path)
{
	...
	char *pwd = strcat(path,"/Cfiles/3DConvolution.cl");
	fp = fopen(pwd, "r");
...
{	
   ...
   // Connect to a compute device
   	...
   	printf(" The program 3DConvolution will execute on : %s \n", query);
...
void cl_launch_kernel(int ni, int nj, int nk)
{
	...
	printf("Execution Time in seconds: ");
 	...
 	FILE *myfile ;
   	...
   	fprintf(myfile,"%s","3DConvolution");
   	fprintf(myfile,"|");
   	fprintf(myfile,"%0.6lf",executionTime);
   	fprintf(myfile,"|");
   	fprintf(myfile,"%d",NI*NJ*NK);
   	fprintf(myfile,"\n");
   	fclose (myfile);
}
...
{	
	...
	init(ni, nj, nk, POLYBENCH_ARRAY(A)); // NI*NJ*NK
