...
void read_cl_file(char* path)
{
	...
	char *pwd = strcat(path,"/Cfiles/gemver.cl");
	fp = fopen(pwd, "r");
	if (!fp) {
...
{	
   ...
   // Connect to a compute device
   	...
   	printf(" The program gemver will execute on : %s \n", query);
...
void cl_launch_kernel(int n, DATA_TYPE alpha, DATA_TYPE beta,char* filename)
{
	...
	printf("Execution Time in seconds:");
 	...
 	FILE *myfile ; 
   	...
   	fprintf(myfile,"%s","gemver");
   	fprintf(myfile,"|");
   	fprintf(myfile,"%0.6lf",executionTime);
   	fprintf(myfile,"|");
   	fprintf(myfile,"%d",N*N+N+N+N+N+N+N+N+N);
   	fprintf(myfile,"\n");
  	fclose (myfile);
}
...
int main(int argc, char *argv[])
{
	...
	cl_load_prog();
	cl_launch_kernel(n, alpha, beta, filename);
